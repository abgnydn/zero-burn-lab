...
def compute_bio_packaging_hub(
    ...
    if opt_seed_trays:
        boost = finished_kg * 0.10 * 35  # 10% output as seed trays at ฿35/kg
        total_revenue += boost
        extra_invest += 2000
    ...
    if opt_egg_cartons:
        boost = finished_kg * 0.10 * 40  # 10% output as egg cartons at ฿40/kg
        total_revenue += boost
        extra_invest += 5000
    ...
    actual_pcs_per_day = finished_after_qc / days_per_year  # actual pieces from real production
    pieces_per_kg_avg = sum(p['pieces_per_kg'] for p in products.values()) / 4
    actual_pcs_per_day_count = actual_pcs_per_day * pieces_per_kg_avg
    ...
    for i, mo in enumerate(months):
        ...
        days_this_mo = round(avg_days_per_mo * pack_avail * straw_avail[i])
        hrs_this_mo = round(days_this_mo * work_hours_per_day)
        pieces = round(days_this_mo * actual_pcs_per_day_count)
    ...
    platform_cut = profit * (revenue_share_pct / 100) if financing_model == 'revenue_share' else 0
    ...
    all_tiers = []
    for key, ti in tiers.items():
        cap = ti['capacity_kg_day'] * hour_scale * days_per_year
        su = min(cap, total_straw)
        ey = ti['yield_pct'] + yield_boost
        pk = su * (ey / 100) * 0.9 * (1 - qc_reject_rate)
        rev = pk * (18 * price_mult)  # use same pricing as current calc
        ...
        all_tiers.append({
            ...
            'capacity': f"{round(ti['capacity_kg_day'] * hour_scale)} kg/day",
...
def _calc_platform_income(model, share_pct, n_hubs, hub_revenue, hub_profit, hub_invest):
    ...
    if model == 'revenue_share':
        owner_per_hub = hub_profit * (share_pct / 100)  # share of PROFIT, not revenue
        farmer_per_hub = hub_profit - owner_per_hub  # farmer gets rest
        owner_invest_per_hub = kit_cost
        owner_description = f"You provide equipment, take {share_pct:.0f}% of profit"
    elif model == 'kit_sale':
    ...
    for yr in range(1, 6):
        price_growth = 1.10 ** (yr - 1)  # 10% annual price growth
        if model == 'kit_sale' and yr > 1:
            yr_income = 0  # kit sale is one-time
        else:
            yr_income = total_owner_income * price_growth
